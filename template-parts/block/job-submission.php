...
if ( ! is_user_logged_in() ) : ?>
	...
	<div class="wpac-alert wpac-alert--warning mceNonEditable">
		<div class="wpac-alert__body mceEditable">
			採用企業や応募方法を見るには<a class="alert-link" href="<?php echo wp_login_url( get_permalink() ); ?>">ログイン</a>してください。
...
<?php else : ?>
			...
			<div class="wp-profile-box">
				...
				<div class="wp-profile-box__container">
					<div class="wp-profile-box__figure">
						<?php
						if ( $attachment = get_term_meta( $company->term_id, 'image', true ) ) {
	...
	<?php if ( $submission = capitalp_get_submission() ) : ?>
		<table>
			...
			<tr>
				<th>採用責任者</th>
				<td><?php echo esc_html( tscfp( '_assignee' ) ); ?></td>
			</tr>
			<?php
			foreach ( [
				'_email' => 'メール',
	...
	<?php elseif ( ! capitalp_job_is_open() ) : ?>
		<div class="wpac-alert wpac-alert--warning mceNonEditable">
			<div class="wpac-alert__body mceEditable">
				この求人は<?php echo mysql2date( get_option( 'date_format' ), tscfp( '_job_expires' ) ); ?>で募集を終了しています。
			</div>
		</div>
	<?php else : ?>
		<p>
