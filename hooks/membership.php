...
// Update license
add_action(
	'ofuse_member_profile_update',
	function ( $user_id, $old_data ) {
...
add_action(
	...
	function () {
		...
		add_settings_field(
			...
			function () {
				?>
				<input type="text" name="capitalp_group_id" id="capitalp_group_id" value="<?php echo esc_attr( get_option( 'capitalp_group_id', '' ) ); ?>" />
				<?php
			},
			'reading',
			'capitalp_membership'
...
add_action(
	...
	function ( $logged_in_cookie, $expire, $expiration, $user_id ) {
...
add_action(
	...
	function () {
		if ( ! wp_next_scheduled( 'capitalp_membership' ) ) {
			wp_schedule_event( time(), 'weekly', 'capitalp_membership' );
		}
	}
);
